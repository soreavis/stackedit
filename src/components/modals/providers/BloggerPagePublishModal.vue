  ...
  <modal-inner aria-label="Publish to Blogger Page">
    <div class="modal__content">
      ...
      <form-entry label="Existing page ID" info="optional">
        <template #field><input class="textfield" type="text" v-model.trim="pageId" @keydown.enter="resolve()"></template>
      </form-entry>
  ...
  methods: {
    resolve() {
      if (!this.blogUrl) {
        ...
        // Return new location
        const location = (bloggerPageProvider as any).makeLocation(
          this.config.token,
          this.blogUrl,
          this.pageId,
        );
