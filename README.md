...
![Tests](https://img.shields.io/badge/tests-247_passing-brightgreen)
...
- **247 specs** across 20 files under `test/unit/`. Hardening specs (`test/unit/hardening/`) cover: sanitizer XSS vectors, rate limiter, API handlers, GitHub OAuth PKCE, template worker sandbox, `vercel.json` contract, markdown-it plugins, drag-and-drop markdown import, mermaid lightbox pan/zoom/copy. Component specs (`test/unit/components/`) cover: `defaultLocalSettings` invariants and the icon registry.
...
247 tests across 20 files under `test/unit/` (hardening + component specs). Paste-ready manual fixtures under `test/fixtures/` for browser smoke-testing (sanitizer XSS vectors, KaTeX, Mermaid, YAML front-matter).
